...
class DownProjectBlock(nn.Module):
    ...
    def forward(self, x_input):
        ...
        ### START CODE HERE
        attn_out = self.attn(self.ln1(x_input), self.ln1(self.C))  
        attn_out = attn_out + self.mlp(self.ln2(attn_out))
        return attn_out
        ### END CODE HERE
...
class UpProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        ### START CODE HERE
        # Layer normalization and cross-attention initialization
        self.ln1 = nn.LayerNorm(config.n_embd)
        self.ln2 = nn.LayerNorm(config.n_embd)
        # Initialize cross-attention layer for up projection
        self.cross_attn = CausalCrossAttention(config)
        
        # Optional MLP layer for further transformation
        self.mlp = nn.Sequential(
            nn.Linear(config.n_embd, 4 * config.n_embd),
            nn.GELU(),
            nn.Linear(4 * config.n_embd, config.n_embd),
    ...
    def forward(self, y, x_input):
        ...
        ### START CODE HERE
        norm_x_input = self.ln1(x_input)
        norm_y = self.ln1(y)
        attn_out = self.cross_attn(norm_y, norm_x_input)
        attn_out = x_input + attn_out
        output = attn_out + self.mlp(self.ln2(attn_out))
        return output
        ### END CODE HERE
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        if self.perceiver:
            ...
            print("After DownProjectBlock shape:", x.shape)  # Expected: [batch_size, bottleneck_dim, n_embd]
        ...
        x = self.blocks(x)
        print("After Transformer Blocks shape:", x.shape)  # Expected: [batch_size, bottleneck_dim, n_embd]

        if self.perceiver:
            x = self.up_block(x, x_input)
        print("After UpProjectBlock shape:", x.shape)  # Expected: [batch_size, seq_len, n_embd]

        x = self.ln_f(x)
        logits = self.head(x)
        print("Logits shape:", logits.shape)  # Should be [batch_size, seq_len, vocab_size]
        if targets is not None:
            print("Targets shape:", targets.shape)  # Should be [batch_size, seq_len]
