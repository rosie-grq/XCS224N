...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        min_length=4
        max_length=int(self.max_context_size)
        random_length=random.randint(min_length,max_length)
        truncated_document=document[:random_length]
        
        doc_length = len(truncated_document)
        avg_mask_length = int(max(1, doc_length * self.masking_percent)) 
        mask_length = random.randint(1, int(avg_mask_length * 2))

        start_idx = random.randint(0, max(0, doc_length - mask_length))
        ...
        masked_string = masked_string[:self.block_size].ljust(self.block_size, self.PAD_CHAR)
