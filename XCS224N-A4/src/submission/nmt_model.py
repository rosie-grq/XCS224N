...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        X_pack=pack_padded_sequence(X, source_lengths, batch_first=False, enforce_sorted=True)
        enc_hiddens,(last_hidden, last_cell) =self.encoder(X_pack)
        enc_hiddens, _ =pad_packed_sequence(enc_hiddens, batch_first=False, padding_value=0.0)
        ...
        dec_init_state = (self.h_projection(last_hidden_cat), self.c_projection(last_cell_cat))  # (b, h)
        ### END CODE HERE
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        batch_size = enc_hiddens.size(0) #get the batch size from the encoder hidden states, dimension 0
        ...
        Y=self.model_embeddings(target_padded,is_target=True) #automatically invoke the forward function
        Y_split = torch.split(Y, 1, dim=0) #split on dim=0 into chunks of size 1
        for Y_t in Y_split:
            Y_t=torch.squeeze(Y_t)   #sqeeze out first dimenions of 1
            Ybar_t=torch.cat((Y_t,o_prev), dim=-1) #concatenate on the last dimension
            #print("Y_t shape:", Y_t.shape)
            #print("o_prev shape:", o_prev.shape)
            #print("Ybar_t shape:", Ybar_t.shape)
            dec_state, o_t, e_t=self.step(Ybar_t,dec_state,enc_hiddens,enc_hiddens_proj,enc_masks)
            combined_outputs.append(o_t)
            o_prev=o_t
        combined_outputs=torch.stack(combined_outputs,dim=0) #difference btw stack and cat, stack create new dimension
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #print("dec_state shapes:", dec_state[0].shape, dec_state[1].shape)
        dec_hidden, dec_cell = dec_state 
        #print("dec_hidden shape:", dec_hidden.shape)
        e_t = torch.bmm(dec_hidden.unsqueeze(1), enc_hiddens_proj.transpose(1, 2)) # #(b,1,h) (b, h,src_len) output: b,1,src_len
        ...
        alpha_t=torch.softmax(e_t,dim=1) #(b,src_length)
        #print("alpha_t shape:", alpha_t.shape)
        a_t=torch.bmm(alpha_t.unsqueeze(1),enc_hiddens).squeeze(1) # (b,1,src_len) (b, src_len, h * 2) --> (b,h*2)
        #a_t is the weighted sum of encoder hidden states, so it has the same dimension of encoder hidden states
    ...
    def generate_sent_masks(self, enc_hiddens: torch.Tensor, source_lengths: List[int]) -> torch.Tensor:
        ...
        for e_id, src_len in enumerate(source_lengths):
            enc_masks[e_id, src_len:] = 1 # only use mask to indicate padded positions
