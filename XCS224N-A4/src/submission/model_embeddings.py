...
class ModelEmbeddings(nn.Module): 
    ...
    def forward(self, x, is_target=False):
        if is_target:
            # Use target embeddings if is_target is True
            return self.target(x)  # (batch_size, seq_len, embed_size)
        else:
            # Use source embeddings otherwise
            return self.source(x)  # (batch_size, seq_len, embed_size)
